...
class Category extends Admin_Controller {
	...
	function index($field='id', $by='DESC', $code=0,$page=0)
	{
		...
		$data['category']	    = $this->Category_model->get_categorys($limit,$page, $field, $by,$term);
		$count_category	    = $this->Category_model->get_categorys(0,$page, $field, $by,$term);

		$this->load->library('pagination');

		$config['base_url']		    = base_url().'/'.$this->config->item('admin_folder').'/category/index/'.$field.'/'.$by.'/'.$code;
		...
		if(!empty($_POST) && $_POST['id'] == 'ajaxloaded') {
		   $this->view_ajax($this->config->item('admin_folder').'/masters/category_ajax', $data);
		}else {
		    $this->view($this->config->item('admin_folder').'/masters/category', $data);
		}
	...
	function form($id = false)
   	{
        ...
        $data['parent_list']        = $this->Category_model->get_parent_list();
        $data['application_list']   = $this->Category_model->get_application_list();
        ...
        {
		    $this->view2($this->config->item('admin_folder').'/masters/category_form', $data);
        }
        else
        {
            ...
            $homebanner_id              = $this->Category_model->save($save);
    ...
    function activate_dactivate()
	{
	    ...
	    { 
	        ...
	        if($category->enabled == 1) {
	            $this->Category_model->activate_dactivate($id,0);
	            //$this->session->set_flashdata('message', 'Deactivated Successfully');
	            $val .= '2';
	        } else {
	            $this->Category_model->activate_dactivate($id,1);
	...
	function get_serial()
	{
	    ...
	    {
	        $serial = $_POST['serial'];
	        $this->Category_model->organize_contents($serial);
        }
	}
