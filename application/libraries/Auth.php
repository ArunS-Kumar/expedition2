...
class Auth
{
    ...
    function check_access_permission($default_redirect=false, $redirect = false)
    {
        $admin = $this->CI->session->userdata('admin');
        ...
        $this->CI->db->select('id')->from('permissions')->where(array('section'=>$uris,'role_id'=>$result->access));
        $followingSet = $this->CI->db->get()->row();
        ...
        if(!empty($followingSet))
        {
            ...
            elseif($default_redirect)
            {
                redirect(config_item('admin_folder').'/access_denied');
            }
            else
            {
                redirect(config_item('admin_folder').'/access_denied');
    ...
    */
    function login_admin($username, $password, $remember=false)
    {
        ...
        $this->CI->db->select('admin.*,roles.name as role');
        $this->CI->db->where('username', $username);
        $this->CI->db->where('password',  sha1($password));
        $this->CI->db->where('admin.enabled', 1);
        $this->CI->db->limit(1);
        $this->CI->db->join('roles','roles.id = admin.access','left');
        $result = $this->CI->db->get('admin');
        ...
        if (sizeof($result) > 0)
        {
            ...
            $admin['admin']['created_on'] = $result['created_on'];
            $admin['admin']['role'] = $result['role'];
