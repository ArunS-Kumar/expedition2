...
class Tags extends Admin_Controller {
	...
	function index($field='id', $by='DESC', $code=0,$page=0)
	{
		...
		$data['tags']	    = $this->Tags_model->get_all_values($limit,$page, $field, $by,$term);
		$count_tags	    = $this->Tags_model->get_all_values(0,$page, $field, $by,$term);

		$this->load->library('pagination');

		$config['base_url']		    = base_url().'/'.$this->config->item('admin_folder').'/tags/index/'.$field.'/'.$by.'/'.$code;
		$config['total_rows']	    = count($count_tags);
		...
		if(!empty($_POST) && $_POST['id'] == 'ajaxloaded') {
		   $this->view_ajax($this->config->item('admin_folder').'/masters/tags_ajax', $data);
		}else {
		    $this->view($this->config->item('admin_folder').'/masters/tags', $data);
		}
	...
	function form($id = false)
   	{
        ...
        if ($id)
        {   
            $tags                 = $this->Tags_model->get_category($id);

            if (!$tags) 
            {
                $this->session->set_flashdata('error', lang('error_not_found'));
                redirect($this->config->item('admin_folder').'/tags');
            }
            
            $data['id']                = $tags->id;
            $data['name']              = $tags->name;
            $data['description']       = $tags->description;
            $data['activate']           = $tags->activate;
        }
        ...
        {
		    $this->view2($this->config->item('admin_folder').'/masters/tags_form', $data);
        }
        else
        {
            ...
            $homebanner_id              = $this->Tags_model->save($save);
    ...
    function activate_dactivate()
	{
	    ...
	    { 
	        ...
	        $tags       = $this->Tags_model->get_category($id);
	        if($tags->activate == 1) {
	            $this->Tags_model->activate_dactivate($id,0);
	            //$this->session->set_flashdata('message', 'Deactivated Successfully');
	            $val .= '2';
	        } else {
	            $this->Tags_model->activate_dactivate($id,1);
	...
	function get_serial()
	{
	    ...
	    {
	        $serial = $_POST['serial'];
	        $this->Tags_model->organize_contents($serial);
        }
	}
