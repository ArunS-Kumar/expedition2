...
class Style extends Admin_Controller {
	...
	function index($field='id', $by='DESC', $code=0,$page=0)
	{
		...
		$data['style']	    = $this->Style_model->get_all_values($limit,$page, $field, $by,$term);
		$count_style	    = $this->Style_model->get_all_values(0,$page, $field, $by,$term);

		$this->load->library('pagination');

		$config['base_url']		    = base_url().'/'.$this->config->item('admin_folder').'/style/index/'.$field.'/'.$by.'/'.$code;
		$config['total_rows']	    = count($count_style);
		...
		if(!empty($_POST) && $_POST['id'] == 'ajaxloaded') {
		   $this->view_ajax($this->config->item('admin_folder').'/masters/style_ajax', $data);
		}else {
		    $this->view($this->config->item('admin_folder').'/masters/style', $data);
		}
	...
	function form($id = false)
   	{
        ...
        if ($id)
        {   
            $style                 = $this->Style_model->get_category($id);

            if (!$style) 
            {
                $this->session->set_flashdata('error', lang('error_not_found'));
                redirect($this->config->item('admin_folder').'/style');
            }
            
            $data['id']                = $style->id;
            $data['name']              = $style->name;
            $data['description']       = $style->description;
            $data['activate']           = $style->activate;
        }
        ...
        {
		    $this->view2($this->config->item('admin_folder').'/masters/style_form', $data);
        }
        else
        {
            ...
            $homebanner_id              = $this->Style_model->save($save);
    ...
    function activate_dactivate()
	{
	    ...
	    { 
	        ...
	        $style       = $this->Style_model->get_category($id);
	        if($style->activate == 1) {
	            $this->Style_model->activate_dactivate($id,0);
	            //$this->session->set_flashdata('message', 'Deactivated Successfully');
	            $val .= '2';
	        } else {
	            $this->Style_model->activate_dactivate($id,1);
	...
	function get_serial()
	{
	    ...
	    {
	        $serial = $_POST['serial'];
	        $this->Style_model->organize_contents($serial);
        }
	}
