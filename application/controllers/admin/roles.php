...
class Roles extends Admin_Controller {
	...
	function index($field='id', $by='DESC', $code=0,$page=0)
	{
		...
		$data['category']	    = $this->Roles_model->get_categorys($limit,$page, $field, $by,$term);
		$count_category	        = $this->Roles_model->get_categorys(0,$page, $field, $by,$term);

		$this->load->library('pagination');

		$config['base_url']		    = base_url().'/'.$this->config->item('admin_folder').'/roles/index/'.$field.'/'.$by.'/'.$code;
		...
		if(!empty($_POST) && $_POST['id'] == 'ajaxloaded') {
		   $this->view_ajax($this->config->item('admin_folder').'/roles/roles_ajax', $data);
		}else {
		    $this->view($this->config->item('admin_folder').'/roles/roles', $data);
		}
	...
	function form($id = false)
   	{
        ...
        if ($id)
        {   
            $category                 = $this->Roles_model->get_category($id);
        ...
        {
		    $this->view2($this->config->item('admin_folder').'/roles/roles_form', $data);
        }
        else
        {
            ...
            $homebanner_id              = $this->Roles_model->save($save);
    ...
    function activate_dactivate()
	{
	    ...
	    { 
	        ...
	        if($category->enabled == 1) {
	            $this->Roles_model->activate_dactivate($id,0);
	            //$this->session->set_flashdata('message', 'Deactivated Successfully');
	            $val .= '2';
	        } else {
	            $this->Roles_model->activate_dactivate($id,1);
	...
	function get_serial()
	{
	    ...
	    {
	        $serial = $_POST['serial'];
	        $this->Roles_model->organize_contents($serial);
        }
	}
