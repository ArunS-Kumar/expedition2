...
class Subgroup extends Admin_Controller {
	...
	function index($field='id', $by='DESC', $code=0,$page=0)
	{
		...
		$data['category']	    = $this->Subgroup_model->get_categorys($limit,$page, $field, $by,$term);
		$count_category	        = $this->Subgroup_model->get_categorys(0,$page, $field, $by,$term);

		$this->load->library('pagination');

		$config['base_url']		    = base_url().'/'.$this->config->item('admin_folder').'/subgroup/index/'.$field.'/'.$by.'/'.$code;
		...
		if(!empty($_POST) && $_POST['id'] == 'ajaxloaded') {
		   $this->view_ajax($this->config->item('admin_folder').'/masters/subgroup_ajax', $data);
		}else {
		    $this->view($this->config->item('admin_folder').'/masters/subgroup', $data);
		}
	...
	function form($id = false)
   	{
        ...
        $data['parent_list']        = $this->Subgroup_model->get_parent_list();
        $data['group_list']         = $this->Common_model->get_groups();
        
        if ($id)
        {   
            $category                 = $this->Subgroup_model->get_category($id);
        ...
        $this->form_validation->set_rules('parent', 'lang:group', 'trim|required');
        $this->form_validation->set_rules('enabled', 'lang:enabled', 'trim|numeric');
        
        if ($this->form_validation->run() == FALSE)
        {
		    $this->view2($this->config->item('admin_folder').'/masters/subgroup_form', $data);
        }
        else
        {
            ...
            $homebanner_id              = $this->Subgroup_model->save($save);
    ...
    function activate_dactivate()
	{
	    ...
	    { 
	        ...
	        if($category->enabled == 1) {
	            $this->Subgroup_model->activate_dactivate($id,0);
	            //$this->session->set_flashdata('message', 'Deactivated Successfully');
	            $val .= '2';
	        } else {
	            $this->Subgroup_model->activate_dactivate($id,1);
	...
	function get_serial()
	{
	    ...
	    {
	        $serial = $_POST['serial'];
	        $this->Subgroup_model->organize_contents($serial);
        }
	}
